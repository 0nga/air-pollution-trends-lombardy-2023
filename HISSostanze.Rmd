...
lombardiaMap <- get_stadiamap((getbb("Lombardia")), maptype = "outdoors", zoom=8 )
```
...
PLOT DETECTION STATION
```{r}
color_scale <- scale_color_manual(values = c("red", "green"))
...
plot <- ggplot() +
  ...
  geom_point(data = stationData, aes(x = lng, y = lat, color= Open), size = 2) +
  labs(x = "Longitude", y = "Latitude") +
  color_scale +
  theme_minimal() +
  theme(legend.position = "top") 
ggsave(plot, file="detectionStation.pdf", width = 5, height = 5)
...
rm(airData)
```
MAPPING + creazione sigla sostanza
```{r}
# Creiamo una tabella di corrispondenza tra i valori e le sigle
tabella_corrispondenza <- data.frame(
  NomeTipoSensore = c("Ammoniaca", "Arsenico", "Benzene","Biossido di Azoto","Biossido di Zolfo","BlackCarbon","Cadmio","Monossido di Azoto","Monossido di Carbonio", "Nikel", "Ossidi di Azoto", "Ozono", "Piombo", "Particelle sospese PM2.5", "PM10 (SM2005)"),
  Sigla = c("NH3", "As", "C6H6", "NO2", "SO2", "BC", "Cd", "NO", "CO", "Ni", "NOX", "O3", "Pb", "PM2.5", "PM10")
)

# Uniamo la tabella di corrispondenza al dataset per aggiungere la colonna 'sigla'
...
plot <- ggplot(fullData, aes(x = Provincia, y = Sigla)) + 
                geom_point() + 
                labs(x = "Zone", y ="Measured Substance")+
                theme_minimal() +  theme(
                axis.text.x = element_text(size = 16),  # Adjust the 'size' parameter as needed
                axis.text.y = element_text(size = 16))
ggsave(plot, file="sostanzeProvince.png", width = 8, height = 5)
...
instanceNumber$Var1 <- c("NH3", "As", "C6H6", "NO2", "SO2", "BC", "Cd", "NO", "CO", "Ni", "NOX", "O3", "Pb", "PM2.5", "PM10")

custom_values <- instanceNumber$Freq
custom_labels <- instanceNumber$Var1[which(instanceNumber$Freq %in% custom_values)]


pdf("sostanzeIstanze.pdf", width = 8, height = 5)  # Adjust width and height as needed
plot <- barplot(instanceNumber$Freq, names.arg = custom_labels, horiz = TRUE, las = 1, col = "transparent", cex.names = 0.9) 
...
plot <- ggplot(monthlyMeanBc, aes(x = Month, y = Mean_Value, group = interaction(IdStazione, Year), color = as.factor(IdStazione), linetype = factor(IdStazione))) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        ...
        legend.position = "top")  # Change the legend position here)
ggsave(plot, file="trendBC.pdf", width = 8, height = 5)
...
ggsave(plot, file="PM10DailyLimit.pdf", width = 8, height = 5)
...
plot <- ggplot(PM10_annual_avg_by_province, aes(x = Provincia, y = Avg_Valore_Province, fill = color)) +
  ...
  theme_minimal() + coord_cartesian(ylim = c(0,50))  + theme(plot.title = element_text(hjust = 0.5)) +
  theme(plot.title = element_text(hjust = 0.5),
        axis.text.x = element_text(size = 14),  # Adjust the 'size' parameter as needed
        axis.text.y = element_text(size = 14),
        axis.title.x = element_text(size = 16),  # Adjust the 'size' parameter as needed
        axis.title.y = element_text(size = 16),
        legend.position = "top")  # Change the legend position here
ggsave(plot, file="AnnualPM10.pdf", width = 8, height = 5) 
```
...
plot <- ggmap(lombardiaMap) +
  stat_density_2d(data = PM10_annual_avg_by_station, aes(x = lng, y = lat, fill = Avg_Valore_Station), 
                 geom = "polygon", alpha = 0.08,  binwidth = 0.05) +  # Add density map
...
ggsave(plot, file="annualDensityPM10Station.pdf", width = 5, height = 5)
...
plot <- ggmap(lombardiaMap) +
   stat_density_2d(data = PM10_annual_avg_by_province, aes(x = lng, y = lat, fill = Avg_Valore_Province), 
                 geom = "polygon", alpha = 0.1, binwidth = 0.05) +  # Add density map
...
ggsave(plot, file="annualDensityPM10Zone.pdf", width = 5, height = 5)
```


```{r}
rm(list = c("PM10OverLimit", "PM10_annual_avg_by_station", "PM10_annual_avg_by_province", "PM10", "plot"))
```
...
ggsave(plot, file="annualPM2.5.pdf", width = 8, height = 5)
...
ggsave(plot, file="annualDensityPM2.5Station.pdf", width = 5, height = 5)
...
ggsave(plot, file="annualDensityPM2.5Zone.pdf", width = 5, height = 5)
```
...
ggsave(plot, file = "NO2DailyLimit.pdf",width = 8, height = 5)
...
ggsave(plot, file = "AnnualNO2.pdf", width = 8, height = 5)
...
ggsave(plot, file = "annualDensityNO2Station.pdf", width = 5, height = 5)
...
ggsave(plot, file = "annualDensityNO2Zone.pdf", width = 5, height = 5)
```
...
ggsave(plot, file="annualDensitySO2Station.pdf", width = 5, height = 5)
...
ggsave(plot, file="annualDensitySO2Zone.pdf", width = 5, height = 5)
```
...
# AVERAGE BY STATION
# Convert 'Data' column to Date type
O3$Data <- as.Date(O3$Data)

# Calculate the annual average grouped by IdStazione for the year 2023
O3_annual_avg_by_station <- O3 %>%
  filter(lubridate::year(Data) == 2023) %>%
  group_by(IdStazione) %>%
  summarise(Avg_Valore_Station = mean(Valore, na.rm = TRUE))
...
# AVERAGE BY PROVINCE
# Calculate the annual average grouped by Provincia using the previously calculated data
O3_annual_avg_by_province <- O3_annual_avg_by_station %>%
  left_join(O3 %>% select(IdStazione, Provincia) %>% distinct(), by = "IdStazione") %>%
  group_by(Provincia) %>%
  summarise(Avg_Valore_Province = mean(Avg_Valore_Station, na.rm = TRUE))

O3_annual_avg_by_station <- merge(O3_annual_avg_by_station, stationData[stationData$NomeTipoSensore == "Ozono",])

# Aggiungo le coordinate delle province
O3_annual_avg_by_province <- merge(O3_annual_avg_by_province, province, by="Provincia")
...
plot <- ggplot(O3OverLimit, aes(x = Provincia, y = TimesAbove120, fill = color)) +
  ...
  labs(
    x = "Province",
    y = "Number of times with O3 8h MMA over limit"
  ) +
...
ggsave(plot, file="O3OverLimit.pdf", width = 8, height = 5)
...
ggsave(plot, file="O3OverLimitDensity.pdf", width = 5, height = 5)
...
# Plot the detection station with color based on Avg_Valore_Station and add density map
plot <- ggmap(lombardiaMap) +
  stat_density_2d(data = O3_annual_avg_by_station, aes(x = lng, y = lat, fill = Avg_Valore_Station), 
                 geom = "polygon",alpha = 0.1,  binwidth = 0.05) +  # Add density map
  geom_point(data = O3_annual_avg_by_station, aes(x = lng, y = lat, fill = Avg_Valore_Station),
             size = 3, alpha = 0.8, shape = 21) +  # shape = 21 for filled circles
  labs(x = "Longitude", y = "Latitude") +
  theme(plot.title = element_text(hjust = 0.5)) +
  color_scale +
...
ggsave(plot, file="annualDensityO3Station.pdf", width = 5, height = 5)


# Plot the detection station with color based on Avg_Valore_Station and add density map
plot <- ggmap(lombardiaMap) +
  stat_density_2d(data = O3_annual_avg_by_province, aes(x = lng, y = lat, fill = Avg_Valore_Province), 
                 geom = "polygon", alpha = 0.1,  binwidth = 0.05) +  # Add density map
  geom_point(data = O3_annual_avg_by_province, aes(x = lng, y = lat, fill = Avg_Valore_Province),
             size = 5, alpha = 0.8, shape = 21) +  # shape = 21 for filled circles
  color_scale + 
  labs(x = "Longitude", y = "Latitude") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
ggsave(plot, file="annualDensityO3Zone.pdf", width = 5, height = 5)
...
# Create a bar plot
plot <- ggplot(COOverLimit, aes(x = Provincia, y = TimesAbove10, fill = COOverLimit$color)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = 25, linetype = "dashed", color = "black") + 
...
ggsave(plot, file="COOverLimit.pdf", width = 8, height = 5)
```
...
ggsave(plot, file="annualBenzene.pdf", width = 8, height = 5)
...
ggsave(plot, file="annualDensityBenzeneStation.pdf", width = 5, height = 5)
...
ggsave(plot, file="annualDensityBenzeneZone.pdf", width = 5, height = 5)
```

```{r}
rm(list = c("benzene", "benzene_Annual_Avg_by_province", "benzene_Annual_Avg_by_station", "plot"))
```
...
# Filter rows where Valore is greater than 20, count distinct days where Valore is greater than 20 for each province
NH3OverLimit <- NH3 %>% 
  filter(Valore > 20)%>%
  group_by(Provincia, Data) %>%
  group_by(Provincia) %>%
...
# Create a bar plot
plot <- ggplot(NH3OverLimit, aes(x = Provincia, y = DaysAbove20)) +
  geom_bar(stat = "identity", fill="green") +
  labs(x = "Province", y = "Number of days with NH3 > 20") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5)) +
  theme(plot.title = element_text(hjust = 0.5),
        axis.text.x = element_text(size = 14),  # Adjust the 'size' parameter as needed
        axis.text.y = element_text(size = 14),
        axis.title.x = element_text(size = 16),  # Adjust the 'size' parameter as needed
        axis.title.y = element_text(size = 16))  # Change the legend position here


ggsave(plot, file="NH3DailyLimit.pdf", width = 8, height = 5)
...
ggsave(plot, file="annualDensityNH3Station.pdf", width = 5, height = 5)
...
ggsave(plot, file="annualDensityNH3Zone.pdf", width = 5, height = 5)
```

```{r}
rm(list = c("NH3", "NH3_annual_avg_by_province", "NH3_annual_avg_by_station", "NH3OverLimit", "plot", "color_scale"))
```
...
# plot the detection station
plot <- ggmap(lombardiaMap) + geom_point(data = NO,
                     aes(x = lng, y = lat), size = 2, alpha = 0.15) + 
                     ggtitle("NO Detection Station") +
                     labs(x = "Longitude", y = "Latitude") +
                     theme(plot.title = element_text(hjust = 0.5))
...
# ANNUAL AVG BY STATION AND BY PROVINCE
# Convert 'Data' column to Date type
NO$Data <- as.Date(NO$Data)

# Calculate the annual average grouped by IdStazione for the year 2023
NO_annual_avg_by_station <- NO %>%
  filter(lubridate::year(Data) == 2023) %>%
  group_by(IdStazione) %>%
  summarise(Avg_Valore_Station = mean(Valore, na.rm = TRUE))

# Calculate the annual average grouped by Provincia
NO_annual_avg_by_province <- NO_annual_avg_by_station %>%
  left_join(NO %>% select(IdStazione, Provincia) %>% distinct(), by = "IdStazione") %>%
  group_by(Provincia) %>%
  summarise(Avg_Valore_Province = mean(Avg_Valore_Station, na.rm = TRUE))

# Adding lat and lng for station
NO_annual_avg_by_station <- merge(NO_annual_avg_by_station, stationData[stationData$NomeTipoSensore == "Monossido di Azoto",])

# Adding lat and lng for provinces
NO_annual_avg_by_province <- merge(NO_annual_avg_by_province, province, by="Provincia")
```
...
# Plot the detection station with color based on Avg_Valore_Station and add density map
plot <- ggmap(lombardiaMap) +
    stat_density_2d(data = NO_annual_avg_by_station, aes(x = lng, y = lat, fill = Avg_Valore_Station), 
                 geom = "polygon", alpha = 0.1,  binwidth = 0.5) +  # Add density map
  geom_point(data = NO_annual_avg_by_station, aes(x = lng, y = lat, fill = Avg_Valore_Station),
             size = 3, alpha = 0.8, shape = 21) +  # shape = 21 for filled circles
  color_scale + 
  labs(x = "Longitude", y = "Latitude") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
ggsave(plot, file="annualDensityNOStation.pdf", width = 5, height = 5)
...
plot <- ggmap(lombardiaMap) +
    stat_density_2d(data = NO_annual_avg_by_province, aes(x = lng, y = lat, fill = Avg_Valore_Province), 
                 geom = "polygon", alpha = 0.1,  binwidth = 0.5) +  # Add density map
  geom_point(data = NO_annual_avg_by_province, aes(x = lng, y = lat, fill = Avg_Valore_Province),
             size = 3, alpha = 0.8, shape = 21) +  # shape = 21 for filled circles
  color_scale + 
  labs(x = "Longitude", y = "Latitude") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
ggsave(plot, file="annualDensityNOZone.pdf", width = 5, height = 5)
```
...
# plot the detection station
plot <- ggmap(lombardiaMap) + geom_point(data = NOX,
                     aes(x = lng, y = lat), size = 2, alpha = 0.15) + 
                     ggtitle("NOX Detection Station") +
                     labs(x = "Longitude", y = "Latitude") +
                     theme(plot.title = element_text(hjust = 0.5))
plot
...
# ANNUAL AVG BY STATION AND BY PROVINCE
# Convert 'Data' column to Date type
NOX$Data <- as.Date(NOX$Data)

# Calculate the annual average grouped by IdStazione for the year 2023
NOX_annual_avg_by_station <- NOX %>%
  filter(lubridate::year(Data) == 2023) %>%
  group_by(IdStazione) %>%
  summarise(Avg_Valore_Station = mean(Valore, na.rm = TRUE))

# Calculate the annual average grouped by Provincia
NOX_annual_avg_by_province <- NOX_annual_avg_by_station %>%
  left_join(NOX %>% select(IdStazione, Provincia) %>% distinct(), by = "IdStazione") %>%
  group_by(Provincia) %>%
  summarise(Avg_Valore_Province = mean(Avg_Valore_Station, na.rm = TRUE))

# Adding lat and lng for station
NOX_annual_avg_by_station <- merge(NOX_annual_avg_by_station, stationData[stationData$NomeTipoSensore == "Ossidi di Azoto",])

# Adding lat and lng for provinces
NOX_annual_avg_by_province <- merge(NOX_annual_avg_by_province, province, by="Provincia")
```
...
# Plot the detection station with color based on Avg_Valore_Station and add density map
plot <- ggmap(lombardiaMap) +
  stat_density_2d(data = NOX_annual_avg_by_station, aes(x = lng, y = lat, fill = Avg_Valore_Station), 
                 geom = "polygon", alpha = 0.08 , binwidth=0.05) +  # Add density map
  geom_point(data = NOX_annual_avg_by_station, aes(x = lng, y = lat, fill = Avg_Valore_Station),
             size = 3, alpha = 0.8, shape = 21) +  # shape = 21 for filled circles
  color_scale + 
  labs(x = "Longitude", y = "Latitude") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
ggsave(plot, file="annualDensityNOXStation.pdf", width = 5, height = 5)
...
# Plot the detection station with color based on Avg_Valore_Station and add density map
plot <- ggmap(lombardiaMap) +
  stat_density_2d(data = NOX_annual_avg_by_province, aes(x = lng, y = lat, fill = Avg_Valore_Province), 
                 geom = "polygon", alpha = 0.1 , binwidth=0.05) +  # Add density map
  geom_point(data = NOX_annual_avg_by_province, aes(x = lng, y = lat, fill = Avg_Valore_Province),
             size = 5, alpha = 0.8, shape = 21) +  # shape = 21 for filled circles
  color_scale + 
  labs(x = "Longitude", y = "Latitude") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
ggsave(plot, file="annualDensityNOXZone.pdf", width = 5, height = 5)
```
...
# Plot the detection station with color based on Avg_Valore_Station and add density map
plot <- ggplot() +
  geom_sf(data = lombardia_provinces, fill = "white", color = "black") +  # White background for provinces
  stat_density_2d(data = NOX_annual_avg_by_station, aes(x = lng, y = lat, fill = Avg_Valore_Station), 
                 geom = "polygon", alpha = 0.2 , bins = 25) +  # Add density map
  geom_point(data = NOX_annual_avg_by_station, aes(x = lng, y = lat, fill = Avg_Valore_Station),
             size = 3, alpha = 0.8, shape = 21) +  # shape = 21 for filled circles
  color_scale + 
  labs(x = "Longitude", y = "Latitude") +
